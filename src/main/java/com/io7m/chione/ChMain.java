...
import com.io7m.chione.internal.commands.ChCheckConfiguration;
import com.io7m.chione.internal.commands.ChCreateHashedPassword;
import com.io7m.chione.internal.commands.ChRunServer;
...
public final class ChMain implements Runnable
{
  private static final Logger LOG = LoggerFactory.getLogger(ChMain.class);
    ...
    final String[] inArgs)
  {
    ...
    final List<CLPCommandConstructorType> commands =
      List.of(
        ChCheckConfiguration::new,
        ChRunServer::new,
        ChCreateHashedPassword::new
      );

    final var configuration =
      CLPApplicationConfiguration.builder()
        .setLogger(LOG)
        .setProgramName("chione")
        .setCommands(commands)
        .setDocumentationURI(URI.create(
          "https://www.github.com/io7m/chione/"))
        .build();
  ...
  public static int mainExitless(
    final String[] args)
  {
    final ChMain cm = new ChMain(args);
    cm.run();
    return cm.exitCode();
  }
